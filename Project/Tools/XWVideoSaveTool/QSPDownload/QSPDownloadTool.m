...
- (QSPDownloadSource *)addDownloadTast:(NSString *)netPath andOffLine:(BOOL)offLine;
{
    ...
    [source setFileName:[self getFileName:[self nameWithURLStr:netPath]]];  //文件格式
    [source setLocation:[QSPDownloadTool_DownloadDataDocument_Path stringByAppendingPathComponent:source.fileName]]; //文件名
    source.style = QSPDownloadSourceStyleDown;
...
//自己添加
- (NSString *)pathWithURL:(NSString *)urlStr
{
    return [QSPDownloadTool_DownloadDataDocument_Path stringByAppendingPathComponent:[self nameWithURLStr:urlStr]];
}
